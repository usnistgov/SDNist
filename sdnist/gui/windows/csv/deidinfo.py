...
class DeidInfoPanel(AbstractPanel):
    def __init__(self,
                 file_path: Path,
                 data: pd.DataFrame,
                 *args,
    ...
    def _create(self):
        ...
        labels_map = {
            "Number of records": str(self.n_rows),
            "Number of features": str(self.n_features)
        }
        all_elems = []
        ...
        for f in self.features:
            ...
            features_map[f] = (name, unique_values, has_N, dtype)
        ...
        feature_head = PartInfoLinePanel(
            parts=['FEATURE NAME','# UNIQUE VALUES',
                   "HAS VALUE \'N\'", "DATA TYPE"],
        ...
        for f in self.features:
            ...
            f_elem = PartInfoLinePanel(
                parts=[f, str(features_map[f][1]),
                       str(features_map[f][2]), features_map[f][3]],
                rect=f_rect,
                container=self.scroll,
                parent_element=self.scroll,
                manager=self.manager,
