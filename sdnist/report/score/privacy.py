...
def privacy_score(dataset: Dataset, ui_data: ReportUIData, report_data, log: SimpleLogger) \
    ...
    if len(quasi_idf) == 0:
        log.msg('No quasi-identifier feature found in the dataset. Skipping Apparent Match Distribution.',
                level=2,
                timed=False,
                msg_type='error')
    ...
    disco_result_plot_outfile = os.path.join(r_ui_d.output_directory, "disco_score_plot.png")
    disco_heatmap_plot_outfile = os.path.join(r_ui_d.output_directory, "disco_heatmap_plot.png")
    disco_evaluator.plot_disco_results(disco_result_plot_outfile)
    disco_evaluator.plot_disco_minus_dio_heatmap(disco_heatmap_plot_outfile)
    disco_para_a = Attachment(name=None,
                              _data=disco_explainer_a,
                              _type=AttachmentType.String)
    disco_para_b = Attachment(name=None,
                              _data=disco_explainer_b,
                              _type=AttachmentType.String)
    disco_plot_a = Attachment(name=None,
                     _data=[{IMAGE_NAME: "Disco Bar Plot", PATH: disco_result_plot_outfile},
                            {IMAGE_NAME: "Disco Heatmap", PATH: disco_heatmap_plot_outfile}],
                     _type=AttachmentType.ImageLinks)

    r_ui_d.add(PrivacyScorePacket("DiSCO Privacy Score",
                                  None,
                                  [disco_para_a, disco_para_b, disco_plot_a]))
    log.end_msg()
