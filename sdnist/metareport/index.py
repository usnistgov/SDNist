...
def reports_from_index(index: pd.DataFrame, filters: Dict[str, Dict],
    ...
    for f_name, filter in filters.items():
        ...
        if f_name.startswith('f'):  # that means inclusion
            ...
            fdf = fdf[~fdf['deid data id'].isin(res_df['deid data id'])]
    ...
    for path in paths:
        ...
        for file in path.glob('**/*.json'):
            ...
            with open(file, 'r') as f:
                ...
                try:
                    report_names = [
                        ('_'.join(str(p.stem).split('_')[1:-2]), p)
                        for p in file.parent.iterdir() if p.is_dir()
                    ]
                    for rn, rp in report_names:
                        if str(file.stem) == rn:
                            report_path = Path(*rp.parts[-4:])
                            break

                    print('REPORT PATH', file.stem, rp.stem)
...
paths = [Path("crc_acceleration_bundle_1.1/crc_data_and_metric_bundle_1.1/deid_data")]
out_dir = Path('crc_acceleration_bundle_1.1/crc_data_and_metric_bundle_1.1')
